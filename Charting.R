...
charting_data <- returns_long %>% filter(Ticker == ticker, Date >= "2021-09-24")
...
candlestick <- ggplot(charting_data) +
  ...
  xlab("Date") + 
  ylab("Stock Price(INR)") +
  labs(
    title = paste0(charting_data$Companies[1], " (", ticker, ")"),
    subtitle = charting_data$Sector[1],
    caption = "Source: Yahoo! Finance"
  ) +
  #scale_y_continuous(labels = scales::dollar) +      #IT CHANGES DOLLAR SYMBOL TO Y AXIS
  theme(
...
price_chart <- ggplot(price_data) +
  ...
  ylab("Stock Price(INR)") +
  labs(
    title = paste0(price_data$Companies[1], " (", ticker, ")"),
...
## Industry Chart
sector <- bsedata %>% filter(Ticker == ticker) %>% select(Sector) %>% as.character()
industry <- bsedata %>% filter(Ticker == ticker) %>% select(Companies) %>% as.character()

industry_summary_data <- performance_summary %>% 
  filter(Sector == sector) %>% 
  mutate(
    isCompanies = ifelse(Companies == Companies, "Industry", "Non_Industry")
  )

industry_chart <- ggplot(industry_summary_data) +
  geom_bar(aes(x = Companies, y = One_year, fill = isCompanies), stat = "summary", fun = "mean") +
...
performance_summary_data <- performance_summary %>% 
  ...
  select(Thirty_days, Ninety_days, One_year, Three_years, Five_years)
...
performance_summary_data$Period <- factor(performance_summary_data$Period, levels = c("1 Month", "1 Quarter", "1 Year", "3 Years", "5 Years"))
