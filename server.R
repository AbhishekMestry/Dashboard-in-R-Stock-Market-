...
server <- function(input, output){
  ...
  output$industry_chart <- renderPlot({
    sector <- bsedata %>% filter(Ticker == input$ticker_select) %>% select(Sector) %>% as.character()
    industry <- bsedata %>% filter(Ticker == input$ticker_select) %>% select(Companies) %>% as.character()
